...
def load_excel(file_bytes):
    ...
    for name in xl.sheet_names:
        ...
        # Normalizar columna de actividad / observación
        for label in ["observacion", "molido", "proceso.1"]:
            col = next((c for c in df.columns if label in c.lower()), None)
            if col:
                df["_actividad"] = df[col]
                break
        else:
            df["_actividad"] = "Sin descripción"
...
with tab1:
    for i, proc in enumerate(selected):
        ...
        with col_b:
            ...
            fig2 = px.histogram(
                df, x="_segundos", nbins=6,
                color_discrete_sequence=[color],
                title="Distribución de tiempos",
                labels={"_segundos": "Segundos"}
            )
